...
def _sleep(seconds: float) -> None:
    scale = float(os.getenv("SYNTHETIC_PHYLO_TIME_SCALE", "1.0"))
    time.sleep(seconds * scale)


def _run_profiled_tool(tool_name: str, operation):
    from examples.commons.synthetic_phylogenetic_subtrees.tool_profiles import TOOL_PROFILES

    profile = TOOL_PROFILES[tool_name]
    _sleep(profile.sleep_seconds)
    if profile.behavior.startswith("mock_"):
        raise RuntimeError(profile.failure_message or f"{tool_name} failed during synthetic execution.")
    return operation()


def run_profiled_validator(tool_name: str, path_to_fasta_files: str):
    return _run_profiled_tool(tool_name, lambda: sequence_validator_(path_to_fasta_files))


def run_profiled_validation_pipeline(tool_name: str, path_to_fasta_files: str) -> str:
    def validate_or_correct() -> str:
        validation_result = sequence_validator_(path_to_fasta_files)
        if validation_result is True:
            return f"The FASTA files are valid. Use path={path_to_fasta_files}."

        corrected_message = fix_invalid_sequences(path_to_fasta_files)
        corrected_path = GENERAL_DIR / "validated-files"
        corrected_validation = sequence_validator_(str(corrected_path))
        if corrected_validation is True:
            return f"The FASTA files were corrected and validated. Use path={corrected_path}."

        return str(corrected_message)

    return _run_profiled_tool(tool_name, validate_or_correct)


def run_profiled_corrector(tool_name: str, path_to_fasta_files: str):
    return _run_profiled_tool(tool_name, lambda: fix_invalid_sequences(path_to_fasta_files))


def run_profiled_alignment(tool_name: str):
    return _run_profiled_tool(tool_name, multiple_sequence_alignment_using_mafft)


def run_profiled_tree_builder(tool_name: str):
    return _run_profiled_tool(tool_name, tree_construction)


def run_profiled_subtree_analyzer(tool_name: str):
    return _run_profiled_tool(tool_name, subtree_generation_and_similarity_calculation)
...
def multiple_sequence_alignment_using_mafft(output_path: str = None) -> str:
    ...
    return f'The sequences were successfully aligned. Use path={output_path}.'
